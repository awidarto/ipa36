<?php if (!defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Documents extends Public_Controller
	{
		...
		function Documents()
		{
			...
			$this->load->model('Filemodel','myfiles');
            ...
            $this->section = 'DC';
            $this->where = null;
            $this->page->set_crumb('Media Library','media/admin');
        ...
        function index()
        {
            ...
            $orderby = ($this->uri->segment(5))?$this->uri->segment(5):"doc_date";
            ...
            $qdf = $this->docs->getOldestDate('Documents','doc_date');
            $odate = $qdf->row();
            $odate = explode(' ',$odate->doc_date);
            ...
            if($from != 'none'){
                ...
                $doptions['yearto'] = null;
                $doptions['monthto'] = null;
                $doptions['dayto'] = null;
            }
            ...
            $fields = 'id,doc_id,doc_uid,doc_number,doc_date,doc_date_create,doc_subject,doc_originator,doc_addressee,doc_company,doc_attention,doc_route,doc_file_id,doc_file_active,doc_loc_main,doc_loc_cab,doc_loc_tray,doc_loc_folder,doc_loc_warehouse,doc_loc_warehouse_box,doc_call_main,doc_call_company,doc_call_date,doc_call_seq,doc_callnumber,doc_classification,doc_retention,doc_keyword,doc_custody,doc_type,doc_group';
            
            $query = $this->docs->fetch('Documents', $fields , $limit , $this->where,array('field'=>$orderby,'dir'=>$sort));            
            
            foreach($query->result() as $doc){
                $doc->doc_route = $this->_getRoutes($doc->id);
                $doc->doc_custody = $this->_getCustody($doc->id);
                $doc->doc_keyword = $this->_getTags($doc->id,$this->section);
                $doc->doc_call_company = $this->_getCompanyName($doc->doc_call_company);
                
                if($doc->doc_file_id !== "no_file"){
                    $active_files = $this->_getActiveFile($doc->doc_file_id,$doc->doc_file_active);
                    if($active_files != false){
                        $doc->active_file = $active_files;
                        if($active_files->mediatype == 'pdf' || $active_files->mediatype == 'msword'){
                            $doc->preview_path = $active_files->thumbnail;
                            $doc->active_preview_files = $this->_getPreviews($active_files->file_path,$active_files->thumbnail);
                        }else{
                            $doc->active_preview_files = false;
                        }
                    }else{
                        $doc->active_file = false;
                    }
                }
        ...
        function samples(){
            // Display Page
    		$this->bep_site->set_crumb('Pesan','media');

    	    if(isset($this->session->userdata['id'])){
        	    $data['currentuser'] = current_user($this->session->userdata['id']);
        	    $data['currentuser']['age'] = user_age($data['currentuser']['dob']);
        	    $data['currentuser']['picture'] = get_avatar($data['currentuser']['id']);
    	    }

    		$data['header'] = "Video Sample";
    		$data['page'] = $this->config->item('backendpro_template_public') . 'samples';
    		$data['module'] = 'media';
    		$this->load->view($this->_container,$data);
        }
