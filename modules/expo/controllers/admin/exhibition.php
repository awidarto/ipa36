<?php if (!defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Exhibition extends Admin_Controller
	{
		...
		function Exhibition()
		{
            ...
            $this->controller = 'expo/admin/exhibition';
            $this->section = 'DC';
            $this->where = null;
            $this->bep_site->set_crumb('Exhibition','expo/admin/exhibition');
        ...
        function index()
        {
            ...
            $orderby = ($this->uri->segment(7))?$this->uri->segment(7):"be_users.id";
            ...
            $query = $this->usermodel->fetch('Booth', '*', null , $this->where,array('field'=>$orderby,'dir'=>$sort));            
            ...
            //print $this->where;
            $query = $this->docs->getUser($this->where, $limit,array('field'=>$orderby,'dir'=>$sort));   
            ...
            foreach($query->result() as $doc){
                ...
                $vid = $this->docs->getUserFile(array('uid'=>$doc->id));        
                
                
                $vids = array();
                foreach($vid->result() as $v){
                    if(file_exists($this->config->item('public_folder').'video/online/'.$v->raw_name.'.flv')){
                        $vids[] = $v;
                    }
                }
                ...
                $ufiles = $this->_getUserFiles($doc->id);
                //print_r($ufiles);
            ...
            if(($type == 'result')){
                $this->bep_site->set_crumb('Search Result','media/admin/usermedia');
            }
            

            $data['header'] = 'User Media';
            if($type == 'xml'){
                $config = array (
                                  'root'    => 'root',
                                  'element' => 'element',
                ...
                $page = $this->config->item('backendpro_template_admin') . "user_list";
                $this->load->view($page,$data);
            }else{
                $data['page'] = $this->config->item('backendpro_template_admin') . "user_list";
                $data['module'] = 'media';
