...
class User extends Public_Controller
{
	...
	function index()
	{
        ...
        */
                ...
                $data['qrfile'] = $this->_generateBarcode($this->session->userdata('id').'_bar.png',$udata);
        		...
        		$data['page'] = $this->config->item('backendpro_template_public') . 'user_profile';
        		$data['module'] = 'user';
	...
	function profile($id = null)
		...
		$data['page'] = $this->config->item('backendpro_template_public') . 'register_success';
		$data['module'] = 'user';
		$this->load->view($this->_container,$data);
	}
	...
	function visitor($id = null)
	{
		...
		$data['page'] = $this->config->item('backendpro_template_public') . 'visitor_success';
		$data['module'] = 'user';
		$this->load->view($this->_container,$data);
	}
	...
	function changepass(){	    
		...
		$this->bep_site->set_crumb('My Profile','user/profile');
		...
		if ( $this->validation->run() === FALSE )
		{
    		...
    		$data['header'] = "Change My Password";
    		$data['page'] = $this->config->item('backendpro_template_public') . 'form_pass_edit';
    		$data['module'] = 'user';
    		$this->load->view($this->_container,$data);
		}
		else
        ...
        function photobooth(){
        	...
        	$data['page'] = $this->config->item('backendpro_template_public') . 'upload_avatar';
        	$data['module'] = 'user';
        	$this->load->view($this->_container,$data);
        }
	...
	function changepicvis($id){
                ...
                if ( $this->validation->run() === FALSE )
                {
            		...
            		$data['page'] = $this->config->item('backendpro_template_public') . 'upload_avatar_vis';
            		$data['module'] = 'user';
            		$this->load->view($this->_container,$data);
		}
		else
...
*/			
	...
	function _updatepass($id)
	{
		...
		if ($this->db->trans_status() === FALSE)
		{
			...
			redirect('user/changepass','location');
		}
		else
		{
			// User registered
			$this->db->trans_commit();
			flashMsg('success',$this->lang->line('userlib_passwd_success'));
			redirect('user/profile','location');
		}
	}
    ...
    function qrdata($data){
        ...
        */
        ...
        //print_r($udata);
        //$udata = implode("\n",$udata);
        //just need the registration number
        $udata = $data['user']['conv_id'];
        return $udata;
    }
