	...
	function fixnumber(){
		...
		foreach($usr as $user){
            ...
            if($user['conv_id'] == ""){
                ...
                $prefix = $prefix[$user['registrationtype']];
                
                $conv_code = str_pad($prefix, 2, "0", STR_PAD_RIGHT);
                
                $user['conv_id'] = $conv_code."-".$sc_code."-000000";
            }
...
*/            
			...
			$data['qrfile'] = $this->_generateBarcode($user['id'].'_bar.png',$udata);
	...
	{
        if($id == null){
            $header = 'Visitor Registration Form';
            $upd = 'create';
        }else{
            $header = 'Update Visitor Profile';
            $upd = 'update';
        }
        
        $validator = $this->_validator_setup_visitor($id,$upd);
		...
		if( ! is_null($id) AND ! $this->input->post('submit'))
		{
			...
			$user = $this->user_model->getVisitors(array('users.id'=>$id));
			$user = $user->row_array();
		...
		if ( $this->validation->run() === FALSE )
		{
			...
			$data['page'] = $this->config->item('backendpro_template_admin') . 'my_form_register_visitor';
			$data['module'] = 'register';
			$this->load->view($this->_container,$data);
		}
		else
		{
			// Submit form
			$this->_register_visitor($id,$upd);
		}
	}


	/**
	 * Register form for officials & participants
	 *
	 * Display the register form to the user
	 *
	 * @access public
	 * @param string $container View file container
	 */
	function official($id = null)
	{
        if($id == null){
            $header = 'Official Registration Form';
            $upd = 'create';
        }else{
        ...
        $validator = $this->_validator_setup_official($id,$upd);
		...
		{
			// Modify form, first load
			$user = $this->user_model->getOfficials(array('users.id'=>$id));
			$user = $user->row_array();
		...
		if ( $this->validation->run() === FALSE )
		{
			...
			$data['upd'] = $upd;
			$data['captcha'] = ($this->preference->item('use_registration_captcha')?$this->_generate_captcha():'');
			$data['page'] = $this->config->item('backendpro_template_admin') . 'my_form_register_official';
			$data['module'] = 'register';
			$this->load->view($this->_container,$data);
		}
		else
		{
			// Submit form
			$this->_register_official($id,$upd);
		}
	}
	...
	function user($id = null)
	{
		...
		if ( $this->validation->run() === FALSE )
		{
			...
			if($upd == 'update'){
			    $this->load->module_view('register','admin/my_form_register',$data);
			}else{
    			$data['page'] = $this->config->item('backendpro_template_admin') . 'my_form_register';
    			$data['module'] = 'register';
    			$this->load->view($this->_container,$data);
			}
		}
	...
	function convention($id = null)
	{
        ...
        $data['get_galadinner_count'] = $this->_get_galadinner_count();
		...
		if( ! is_null($id) AND ! $this->input->post('submit'))
		{
			...
			$header = 'Convention Info - '.$user['fullname'];
		...
		if ( $this->validation->run() === FALSE )
		{
			...
			$data['captcha'] = ($this->preference->item('use_registration_captcha')?$this->_generate_captcha():'');
			$data['module'] = 'register';
            $this->load->module_view('register','admin/my_form_register_2_conv',$data);
		}
		else
		{
			// Submit form
	...
	function shortcourses($id = null)
	{
		...
		if( ! is_null($id) AND ! $this->input->post('submit'))
		{
			...
			$header = 'Short Courses Info - '.$user['fullname'];
		...
		if ( $this->validation->run() === FALSE )
		{
			...
			$data['captcha'] = ($this->preference->item('use_registration_captcha')?$this->_generate_captcha():'');
			$data['module'] = 'register';
            $this->load->module_view('register','admin/my_form_register_2_sc',$data);
		}
		else
		{
			// Submit form
	...
	function _set_profile_defaults()
	{
		...
		$this->validation->set_default_value('conv_id','');
	    ...
	    $this->validation->set_default_value('city','');                                         
        $this->validation->set_default_value('zip','');     
		$this->validation->set_default_value('country','Indonesia');
		$this->validation->set_default_value('registertype','member_domestic');
		$this->validation->set_default_value('registrationtype','Committee');
	...
	function _validator_setup($id = null,$is_update = null){
		...
		if($this->preference->item('allow_user_profiles')){
    		...
    		$fields['mobile'] = 'Cellphone';
    		$fields['company'] = 'Company';
    		$fields['position'] = 'Position';
    		$fields['conv_id'] = 'Registration Number';
	...
	function _validator_setup_visitor($id = null,$is_update = null){
	    // Setup fields
	    $fields['id'] = "ID";
		$fields['username'] = $this->lang->line('userlib_username');
		$fields['password'] = $this->lang->line('userlib_password');
	...
	function _validator_setup_official($id = null,$is_update = null){
		
		$fields['firstname'] = 'First Name';
		$fields['lastname'] = 'Last Name';
		$fields['salutation'] = 'Salutation';
		$fields['company'] = 'Company';
		$fields['registrationtype'] = 'Role';
	 ...
	 * @access private
	 */
	function _register_visitor($id = null, $is_update = null)
	{
		// Build
		//print $is_update;
		//print_r($_POST);
		...
		$data['user_profiles']['email'] = $this->input->post('email');
		$data['user_profiles']['created'] = date("Y-m-d H:i:s",time());
		...
		$data['user_profiles']['registrationtype '] = 'Visitor';
        ...
        if( is_null($id)){
    		...
    		$this->user_model->insert('Visitors',$data['user_profiles']);
    		
    		$id = $this->db->insert_id();
    		
    		$conv_id = 'VST-'.str_pad($id, 6, "0", STR_PAD_LEFT);
    		
    		$this->user_model->update('Visitors',array('conv_id'=>$conv_id),array('id'=>$id));
        }else{
    		$data['user_profiles']['modified'] = date("Y-m-d H:i:s",time());
			...
			// See bug #51
			$data['users_profiles']['conv_id'] = 'VST-'.str_pad($id, 6, "0", STR_PAD_LEFT);
    		
			$this->user_model->update('Visitors',$data['user_profiles'],array('id'=>$id));
		...
		if ($this->db->trans_status() === FALSE)
		{
			...
			redirect('register/admin/register/visitor','location');
		}
		else
		{
			// User registered
			...
			*/
			...
			redirect('media/admin/visitors','location');

		}
	}
    
	function _register_official($id = null, $is_update = null)
	{
		// Build
		//print $is_update;
		//print_r($_POST);
		...
		$data['users_profiles']['email'] = $this->input->post('email');
		...
		$type_prefix = $this->config->item('off_prefix');
		$type_prefix = $type_prefix[$this->input->post('registrationtype')];
        ...
        if( is_null($id)){
            $data['users_profiles']['created'] = date("Y-m-d H:i:s",time());
    		
    		$this->db->trans_begin();
    		// Add user details to DB
            $data['user_profiles']['created'] = date('Y-m-d H:i:s');

    		// Add user_profile details to DB
    		$this->user_model->insert('Officials',$data['user_profiles']);
    		
    		$id = $this->db->insert_id();
    		
    		$conv_id = str_pad($id, 6, "0", STR_PAD_LEFT);
    		$conv_id = $type_prefix.'-'.$conv_id;
    		
    		$this->user_model->update('Officials',array('conv_id'=>$conv_id),array('id'=>$id));
    		
        }else{
            $data['user_profiles']['modified'] = date('Y-m-d H:i:s');

			$this->db->trans_begin();

    		$conv_id = str_pad($id, 6, "0", STR_PAD_LEFT);
    		$conv_id = $type_prefix.'-'.$conv_id;
			$data['user_profiles']['conv_id'] = $conv_id;

			//$data['user_profiles']['conv_id'] = $type_prefix.'-'.str_pad($id, 6, "0", STR_PAD_LEFT);

			$this->user_model->update('Officials',$data['user_profiles'],array('id'=>$id));
		...
		if ($this->db->trans_status() === FALSE)
		{
			...
			if($is_update == 'update'){
    			...
    			redirect('register/admin/register/official','location');
			}
		}
		else
		{
			...
			*/
			...
			redirect('media/admin/officials','location');
		}
	}
	...
	function _register_individual($id = null, $is_update = null)
	{
		...
		$data['user_profiles']['salutation'] = $this->input->post('salutation');
		$data['user_profiles']['conv_id'] = $this->input->post('conv_id');
		...
		*/
        ...
        if( is_null($id)){
    		...
    		if($data['user_profiles']['conv_id'] == "" || $this->checkregnum($data['user_profiles']['conv_id']) == true){
        		$data['user_profiles']['conv_id'] = "00-00-".str_pad($id, 6, "0", STR_PAD_LEFT);
    		}else{
    		    $cvid = explode("-",$data['user_profiles']['conv_id']);
    		    if($cvid[0] != "00"){
    		        $data['user_profiles']['conv_seq'] = (int) $cvid[2];
    		    }else if($cvid[0] == "00" && $cvid[1] != "00"){
    		        $data['user_profiles']['conv_seq'] = 0;
    		        $data['user_profiles']['sc_seq'] = (int) $cvid[2];
    		    }
    		}
		...
		if ($this->db->trans_status() === FALSE)
		{
			...
			/*
			if($is_update == 'update'){
    			redirect('register/admin/register/user','location');
			}else{
			...
			*/
			...
			if($is_update == 'create'){
    			flashMsg('error',$this->lang->line('userlib_registration_failed'));
    			redirect('media/admin/convex','location');
			}else{
			    $data['header'] = 'Update Result';
        		
			    $data['msg'] = ($is_update == 'update')?'User successfully registered':$this->lang->line('userlib_registration_success');
                $this->load->module_view('register','admin/info_success',$data);
			}
...
//			    $this->user_email->send($data['users']['email'],$this->lang->line('userlib_email_register'),'public/email_register',$edata);
                flashMsg('success',($is_update == 'update')?'User profile updated successfully.':'User created successfully.');
                redirect('media/admin/convex','location');
			}else{
			    $data['header'] = 'Update Result';
	...
	function _register_convention($id = null, $is_update = null)
	{
		...
		*/
		...
		if ($this->db->trans_status() === FALSE)
		{
			...
			$data['msg'] = $this->lang->line('userlib_registration_failed');
...
//		$data['user_profiles']['conv_id'] = substr_replace($this->input->post('conv_id'),$sc_code,3,0);
    ...
    function ajaxregnum(){
	    ...
	    $usr = $this->user_model->getUsers(array('profiles.conv_id'=>"'".$r."'"));
	    
	    $lastq = $this->db->last_query();
	    if($usr->num_rows() > 0){
	        $usr = $usr->row();
	        print json_encode(array('reged'=>true,'msg'=>$usr->firstname.' '.$usr->lastname,'lastq'=>$lastq));
	    }else{
	        print json_encode(array('reged'=>false,'msg'=>'Registration Number Unused','lastq'=>$lastq));
	    }
    }
    
    function checkregnum($r){
	    $usr = $this->user_model->getUsers(array('profiles.conv_id'=>"'".$r."'"));
	    if($usr->num_rows() > 0){
	        return true;
	    }else{
	...
	function _createnotification($id){
        ...
        $convention = array(
            ...
            'galadinner' => ($user['galadinner'] == 0)?'No':'IDR '.number_format($user['galadinner'] + $user['galadinneraux']),
            'galadinneraux' => ($user['galadinneraux'] == 0)?'No':'Yes',
	...
	function _get_galadinner_count(){

        $query = $this->user_model->getUsers('profiles.galadinner > 0 and users.group = 1 ');
        $galadinner = $query->num_rows();

        $query = $this->user_model->getUsers('profiles.galadinner > 0 and profiles.galadinneraux > 0 and users.group = 1 ');
        $galadinneraux = $query->num_rows();

        $query = $this->user_model->fetch('Offline','param_val',null,array('param_name'=>'galadinner'));
    	$g = $query->row_array();
    	$galadinneroffline = $g['param_val'];


        $galadinner_count = $galadinner + $galadinneraux;        
        return $galadinner_count;
    }
