...
class Register extends Public_Controller
{
	...
	function _register_convention($id = null, $is_update = null)
	{
		...
		*/
		...
		else
		{
		    ...
		    $this->user_email->send($user['email'],'Convention Registration','public/email_convention',$edata);
	...
	function _register_shortcourses($id = null, $is_update = null)
	{
		...
		*/
        ...
        if( is_null($id)){
			...
			//$data['user_profiles']['conv_id'] = substr_replace($usr['conv_id'],$prefix,0,1);
			$data['user_profiles']['conv_id'] = substr_replace($usr['conv_id'],$this->config->item('shortcourse_prefix'),1,1);
		...
		else
		{
		    ...
		    $this->user_email->send($user['email'],'Short Courses Registration','public/email_sc',$edata);
	...
	function ajaxcancel(){
	    ...
	    if($bo['orderstatus'] != 'open' && $bo['orderby'] == $c){
		    ...
		    $this->user_email->send($user['email'],$subject,'public/email_cancel',$edata);
