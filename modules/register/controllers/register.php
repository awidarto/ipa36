...
class Register extends Public_Controller
{
	...
	function checkin(){
	    $txtin = $this->input->post('txtin');
	    $station = $this->input->post('station');
	    
	    if(preg_match("/^<</",$txtin)){
    	    print json_encode(array('status'=>'ERR','result'=>$txtin));
	    }else{
	        $dat = explode("\n",$txtin);
	        $id = $dat[0];
	        $regnum = $dat[3];
	        ...
	        if($role == 'Visitor'){
	            //check database visitor
	            $url = site_url('media/admin/media/contactlogvis/'.$id.'/'.time());
	            $purl = site_url('media/admin/media/printbadgevis/'.$id.'/'.time());
	        }else if( $role == 'Professional Domestic' || $role == 'Professional Overseas' || $role == 'Student Domestic' || $role == 'Student Overseas'){
	            //check database official
	            $url = site_url('media/admin/media/contactlog/'.$id.'/'.time());
	            $purl = site_url('media/admin/media/printbadge/'.$id.'/'.time());
	        }else{
	            //check database convention
	            $url = site_url('media/admin/media/contactlogoff/'.$id.'/'.time());
	            $purl = site_url('media/admin/media/printbadgeoff/'.$id.'/'.time());
	        }
            $result = array('url'=>$url,'purl'=>$purl);
            
            $data = array(
                'check_time'=>time(),
                'check_datetime'=>date('Y-m-d h:m:s',time()),
            ...
            $this->user_model->insert('Checkin',$data);
	...
	function hcheckin(){
	    ...
	    if(preg_match("/^<</",$txtin)){
	        ...
	        $raw = preg_match("/^[0-9]*/",$txtin,$pr);
	        $id = $pr[0];
	        
	        if(preg_match("/Visitor/",$txtin)){
	            //check database visitor
	            $url = site_url('media/admin/media/contactlogvis/'.$id.'/'.time());
	            $purl = site_url('media/admin/media/printbadgevis/'.$id.'/'.time());
	            $regtype = 'Visitor';
	        }else if( preg_match("/Professional Overseas/",$txtin) || preg_match("/Professional Domestic/",$txtin) || preg_match("/Student Overseas/",$txtin) || preg_match("/Student Domestic/",$txtin) || preg_match("/Sith/",$txtin)){
	            //check database official
	            $url = site_url('media/admin/media/contactlog/'.$id.'/'.time());
	            $purl = site_url('media/admin/media/printbadge/'.$id.'/'.time());
	            $regtype = 'Attendee';
	...
	function qcheckin(){
	    ...
	    if(preg_match("/^<</",$txtin)){
	        ...
	        if(preg_match("/Visitor/",$txtin)){
	            ...
	            $purl = site_url('media/admin/media/printbadgevis/'.$id.'/'.time());
	            $regtype = 'Visitor';
	            $html = $this->getscanneduser($id,$regtype);
	        ...
	        }else if(preg_match("/Official/",$txtin)){
	            //check database convention
	            $url = site_url('media/admin/media/contactlogoff/'.$id.'/'.time());
	            $purl = site_url('media/admin/media/printbadgeoff/'.$id.'/'.time());
	            $regtype = 'Official';
            ...
            $this->user_model->insert('Checkin',$data);
    	    print json_encode(array('status'=>'OK','result'=>$result,'html'=>$html));
	    }
	    
	}

    function getscanneduser($id,$section){
        if($section == 'Visitor'){
            $res = $this->user_model->getVisitors(array('id'=>$id));
            $picture = get_avatar($id,$this->config->item('public_folder').'avatar/','.jpg','','_vis_sm');
        }else if($section == 'Official'){
            $res = $this->user_model->getOfficials(array('id'=>$id));
            $picture = get_avatar($id,$this->config->item('public_folder').'avatar/','.jpg','','_off_sm');
        }else if($section == 'Attendee'){
            $res = $this->user_model->getUsers(array('users.id'=>$id));
            $picture = get_avatar($id,$this->config->item('public_folder').'avatar/');
        }
        
        if($res->num_rows() > 0){
            $row = $res->row();
            $html = '<table width="100%"  style="font-size:18px;font-weight:bold;"><tr><td style="vertical-align:top;font-weight:bold;width:100px;">';
            $html .= '<img style="width:180px;height:240px;" src="'.base_url().'public/avatar/'.$picture.'?'.time().'" /><br />';
            $html .= '</td>';
            $html .= '<td style="vertical-align:top;font-weight:bold;">';
            $html .= $row->fullname.'<br />';
            $html .= $row->company.'<br />';
            $html .= $row->conv_id.'<br />';
            $html .= '<span style="font-size:25px;font-weight:bold;color:maroon;">'.$row->registrationtype.'</span><br />';
            /*
            if($section == 'Attendee'){
                $c = ($row->course_1 > 0)?'Yes':'No';
                $html .= 'Course 1 :'.$c.'<br />';
                $c = ($row->course_2 > 0)?'Yes':'No';
                $html .= 'Course 2 :'.$c.'<br />';
                $c = ($row->course_3 > 0)?'Yes':'No';
                $html .= 'Course 3 :'.$c.'<br />';
                $c = ($row->course_4 > 0)?'Yes':'No';
                $html .= 'Course 4 :'.$c.'<br />';
                $c = ($row->course_5 > 0)?'Yes':'No';
                $html .= 'Course 5 :'.$c.'<br />';
	...
	function manualcheckin(){
        ...
        if($regtype == 'Visitor'){
            //check database visitor
            $usr = $this->user_model->getVisitors(array('conv_id'=>$regnumber));
    		$usr = $usr->row_array();
    		
    		if(isset($usr['id'])){
                $url = site_url('media/admin/media/contactlogvis/'.$usr['id'].'/'.time());
                $purl = site_url('media/admin/media/printbadgevis/'.$usr['id'].'/'.time());
                $status = 'OK';
                $result = array('url'=>$url,'purl'=>$purl);
	            $html = $this->getscanneduser($usr['id'],$regtype);
    		}else{
                $status = 'ERR';
                $result = array('Visitor not found');
    		}
        }else if( $regtype == 'Attendee'){
            //check database official
            $regnumber = trim($regnumber);
            $usr = $this->user_model->getUsers(array('profiles.conv_id'=>"'".$regnumber."'"));
    		$usr = $usr->row_array();

    		if(isset($usr['id'])){
                $url = site_url('media/admin/media/contactlog/'.$usr['id'].'/'.time());
                $purl = site_url('media/admin/media/printbadge/'.$usr['id'].'/'.time());
                $status = 'OK';
                $result = array('url'=>$url,'purl'=>$purl);
	            $html = $this->getscanneduser($usr['id'],$regtype);
                ...
                $status = 'ERR';
                $result = array('Convention Attendee not found');
    		}
        }else if( $regtype == 'Official'){
            //check database convention
            $usr = $this->user_model->getOfficials(array('conv_id'=>$regnumber));
    		$usr = $usr->row_array();
    		if(isset($usr['id'])){
                $url = site_url('media/admin/media/contactlogoff/'.$usr['id'].'/'.time());
                $purl = site_url('media/admin/media/printbadgeoff/'.$usr['id'].'/'.time());
                $status = 'OK';
                $result = array('url'=>$url,'purl'=>$purl);
	            $html = $this->getscanneduser($usr['id'],$regtype);
    		}else{
                $status = 'ERR';
                $result = array('Official Member not found');
    		}
        }
	    ...
	    print json_encode(array('status'=>$status,'result'=>$result, 'html'=>$html));
...
*/            
	...
	function visitor($id = null)
	{
		...
		if ( $this->validation->run() === FALSE )
		{
			...
			$this->bep_site->set_crumb('Visitor Registration Form','register/visitor');

			// Display page
			$data['header'] = $header;
			$data['upd'] = $upd;
			$data['captcha'] = ($this->preference->item('use_registration_captcha')?$this->_generate_captcha():'');
			$data['page'] = $this->config->item('backendpro_template_public') . 'my_form_register_visitor';
			$data['module'] = 'register';
			$this->load->view($this->_container,$data);
		}
		else
	...
	function _register_visitor($id = null, $is_update = null)
	{
        ...
        if( is_null($id)){
			...
			{
				$this->user_model->update('Visitors',$data['user_profiles'],array('user_id'=>$data['users']['id']));
			}
        }
		...
		if ($this->db->trans_status() === FALSE)
		{
			...
			if($is_update == 'update'){
    			...
    			redirect('register/visitor','location');
			}
		}
		else
		{
