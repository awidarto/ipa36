...
<script language="javascript">
    ...
    function checkRegNum(){
        var r = $("#conv_id").val();
        if(r == ""){
            alert("Please fill Registration Number field.");
        }else{
            $.post("<?php print site_url('register/admin/register/ajaxregnum'); ?>", 
                { 'r':r },
                function(data){
                    if(data.reged == true){
                        alert("Registration Number belongs to " + data.msg);
                    }else{
                        alert(data.msg);
                    }
                },'json'
    ...
    <?php
        $lock = ($upd == 'update')?'disabled':'';
        $hid = ($upd == 'update')?'':'disabled';
    ?>
    
<?php print form_open('register/admin/register/user/'.$id,array('class'=>'horizontal'))?>
    <fieldset>
        <ol>
            <li>
                <div class="annotation">(*) mandatory field</div>
                <div class="annotation">(**) fill in to use previous reg number, otherwise leave blank</div>
            </li>
            <li>
                <?php print form_label('Registration Number :*','conv_id')?>
                <?php print form_input('conv_id'.$lock,$this->validation->conv_id,'id="conv_id" class="text" '.$lock)?>
                <?php print form_hidden('conv_id'.$hid,$this->validation->conv_id);?>
                <span class="annotation" style="cursor:pointer" id="checkRegNum" onClick="javascript:checkRegNum();">Check</span>
            </li>
            <li>
