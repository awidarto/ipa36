<?php if (!defined('BASEPATH')) exit('No direct script access allowed');
     ...
     */
	class Attendanceoff extends Admin_Controller
	{
		/**
		 * Constructor
		 */
		function Attendanceoff()
		{
            ...
            $this->controller = 'media/admin/attendanceoff';
            $this->section = 'Official Attendance';
            $this->header = 'Official Attendance Log';
            $this->where = null;
            $this->bep_site->set_crumb('Official Attendance Log','media/admin/attendanceoff');
        ...
        function index()
        {
            ...
            $convql .= ' or ( registertype > 0 and (course_1 + course_2 + course_3 + course_4 + course_5 > 0))'; 
            */
            $convql = " (users.registrationtype = 'Official' ) "; 
            ...
            $query = $this->usermodel->getUserAttendancesOff($this->where, null,array('fields'=>$orderby,'order'=>$sort));   
            ...
            $query = $this->usermodel->getUserAttendancesOff($this->where, $limit,array('fields'=>$orderby,'order'=>$sort));   
            ...
            if(($type == 'result')){
                $this->bep_site->set_crumb('Search Result','media/admin/attendanceoff');
            }
            ...
            if($type == 'xml'){
                ...
                $page = $this->config->item('backendpro_template_admin') . "attendance_list_detailed_off";
                $this->load->view($page,$data);
            }else{
                $data['page'] = $this->config->item('backendpro_template_admin') . "attendance_list_detailed_off";
                $data['module'] = 'media';
