<?php if (!defined('BASEPATH')) exit('No direct script access allowed');
     ...
     */
	class Attendancevis extends Admin_Controller
	{
		/**
		 * Constructor
		 */
		function Attendancevis()
		{
            ...
            $this->controller = 'media/admin/attendancevis';
            $this->section = 'Visitor Attendance';
            $this->header = 'Visitor Attendance Log';
            $this->where = null;
            $this->bep_site->set_crumb('Visitor Attendance Log','media/admin/attendancevis');
        ...
        function index()
        {
            ...
            $orderby = ($this->uri->segment(7))?$this->uri->segment(7):"users.check_datetime";
            ...
            $convql .= ' or ( registertype > 0 and (course_1 + course_2 + course_3 + course_4 + course_5 > 0))'; 
            */
            $convql = " (users.registrationtype = 'Visitor' ) "; 
            ...
            $query = $this->usermodel->getUserAttendancesVis($this->where, null,array('fields'=>$orderby,'order'=>$sort));   
            ...
            $query = $this->usermodel->getUserAttendancesVis($this->where, $limit,array('fields'=>$orderby,'order'=>$sort));   
            ...
            if(($type == 'result')){
                $this->bep_site->set_crumb('Search Result','media/admin/attendancevis');
            }
            ...
            if($type == 'xml'){
                ...
                //$selview = (check('Access Control',NULL,FALSE))?"convention_list_detailed":"convention_list";
                $page = $this->config->item('backendpro_template_admin') . "attendance_list_detailed";
                $this->load->view($page,$data);
            }else{
                $data['page'] = $this->config->item('backendpro_template_admin') . "attendance_list_detailed_vis";
                $data['module'] = 'media';
