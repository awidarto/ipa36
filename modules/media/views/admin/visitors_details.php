...
function delUser(id){
	...
	if (del_ok){
		$.post("<?php print site_url('media/admin/visitors/delusr'); ?>", 
            { 'userid':id },
            function(data){
                alert(data.msg);
                if(data.id == id){
...
<div class="buttons">                
	<a href="<?php print  site_url('register/admin/register/visitor')?>">
...
<table class="data_grid" cellspacing="0"  valign="middle" >
    <thead>
        <tr>
            ...
            <th class="sidepad top center" style="width:150px;">Company</th>
...
<?php if(isset($documents)): ?>
    <tbody>
        <?php foreach($documents as $row):
            ...
            $delete = '';
            $picture = get_avatar($row->id,$this->config->item('public_folder').'avatar/','.jpg','','_vis_sm');
        ...
        <tr class="left_align" id="<?=$row->id;?>">
            ...
            <td class="middle top" style="padding:5px;width:200px;">
                <a class="preview" href="<?=site_url('/media/admin/media/contactlogvis/'.$row->id.'/'.time());?>">
                    <?=$row->firstname.' '.$row->lastname;?>
                </a><br />
                <?=$row->conv_id?><br />
                <a class="uploadpic" href="<?=site_url('/media/admin/media/uploadpicvis/'.$row->id.'/'.time());?>">
                    <?php print $this->bep_assets->icon('picture_add')?>Upload Photo
                </a><br />
                <a class="snappic" href="<?=site_url('/media/admin/media/snappicvis/'.$row->id.'/'.time());?>">
                    <?php print $this->bep_assets->icon('camera')?>Take Photo
                </a><br />
                <a target="_blank" href="<?=site_url('/media/admin/media/printbadgevis/'.$row->id.'/'.time());?>">
                    <?php print $this->bep_assets->icon('printer')?>Print Badge
                </a><br /><br />
                
            	<a href="<?php print  site_url('register/admin/register/visitor/'.$row->id )?>">
                    <?php print  $this->bep_assets->icon('pencil');?>Update Visitor
                </a>
