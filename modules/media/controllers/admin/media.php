<?php if (!defined('BASEPATH')) exit('No direct script access allowed');
	...
	function changepicoff($id){
	    ...
	    if ( $this->validation->run() === FALSE )
		{
			...
			// Display page
            $user= $this->usermodel->getOfficials(array('id'=>$uid));
            $users = $user->row_array();
            $data['user'] = $users;
            $data['id'] = $uid;

            $this->load->module_view('media','admin/uploadpicoff_overlay',$data);
		...
		else
		{
			...
			$data['picture'] = get_avatar($id,$this->config->item('public_folder').'avatar/','.jpg','','_off_sm');
	...
	function changepicvis($id){
	    ...
	    if ( $this->validation->run() === FALSE )
		{
			...
			// Display page
            $user= $this->usermodel->getVisitors(array('id'=>$uid));
            $users = $user->row_array();
            $data['user'] = $users;
            $data['id'] = $uid;

            $this->load->module_view('media','admin/uploadpicvis_overlay',$data);
		...
		else
		{
			...
			$data['picture'] = get_avatar($id,$this->config->item('public_folder').'avatar/','.jpg','','_vis_sm');
        ...
        function printbadge($uid){
            ...
            //ipa 36th use barcode 128
            $data['qrfile'] = $this->_generateBarcode($data['user']['id'].'_bar.png',$udata);
            
            $this->load->module_view('media','admin/print_overlay',$data);
        }

        function printbadgeoff($uid){
            
            $user = $this->usermodel->getOfficials(array('id'=>$uid)); 
            $users = $user->row_array();
            $data['user'] = $users;
			//$data['user']['picture'] = get_avatar($data['user']['user_id'],$this->config->item('public_folder').'avatar/','.jpg','','_sm');
			$data['user']['picture'] = get_avatar($data['user']['id'],$this->config->item('public_folder').'avatar/','.jpg','','_off_sm');
            
            //$udata = $this->_qr_data($data);
            $udata = $this->qrdata($data);
            
			//$data['qrfile'] = $this->_generateQR($data['user']['id'].'_off_qr.png',$udata);
            //ipa 36th use barcode 128
            $data['qrfile'] = $this->_generateBarcode($data['user']['id'].'_off_bar.png',$udata);
        ...
        function printbadgevis($uid){
            ...
            $data['user'] = $users;
			//$data['user']['picture'] = get_avatar($data['user']['user_id'],$this->config->item('public_folder').'avatar/','.jpg','','_sm');
			$data['user']['picture'] = get_avatar($data['user']['id'],$this->config->item('public_folder').'avatar/','.jpg','','_vis_sm');
            ...
            //ipa 36th use barcode 128
            $data['qrfile'] = $this->_generateBarcode($data['user']['id'].'_vis_bar.png',$udata);
        ...
        function snappicvis($uid){
            ...
            $user= $this->usermodel->fetch('Visitors', '*', null, array('id'=>$uid));
            $users = $user->row_array();
            $data['user'] = $users;
            $data['id'] = $uid;
        ...
        function snappicoff($uid){
            
            $user= $this->usermodel->fetch('Officials', '*', null, array('id'=>$uid));
            $users = $user->row_array();
            $data['user'] = $users;
            $data['id'] = $uid;
            
            $this->load->module_view('media','admin/snap_overlay_off',$data);
        }

        
        function receivephoto($id){
            
            $filename = $this->config->item('public_folder').'avatar/'.$id.'.jpg';
            ...
            if (!$result) {
                ...
                $config['width'] = 320;
                $config['height'] = 240;
                $config['new_image'] = $this->config->item('public_folder').'avatar/'.$id.'.jpg';

                $this->image_lib->initialize($config);
                $this->image_lib->resize();

			    $config['image_library'] = 'gd2';
                $config['source_image'] = $this->config->item('public_folder').'avatar/'.$id.'.jpg';
                ...
                $config['new_image'] = $this->config->item('public_folder').'avatar/'.$id.'_sm.jpg';
                $config['maintain_ratio'] = FALSE;
                $config['width'] = 180;
                $config['height'] = 240;
                $config['x_axis'] = (320 - 180) / 2;
            ...
            $url = base_url().'public/avatar/'.$id.'.jpg?'.time();
            print $url;
        }
    	...
    	function delusr()
    	{
			...
			{	// Delete as long as its not the Administrator account
				$this->docs->delete('Users',array('id'=>$id));

				$user= $this->docs->fetch('Users', '*', null, array('id'=>$id));
				
				if($user->num_rows() > 0){
    			    $result = array('id'=>$id,'status'=>'ERR','msg'=>'fail to delete user');
				}else{
    				$this->docs->delete('UserProfiles',array('user_id'=>$id));
        ...
        function invoiceoff($uid){
            $user= $this->usermodel->fetch('Officials', '*', null, array('id'=>$uid));
            $users = $user->row_array();
            $data['user'] = $user->row_array();

            $this->load->module_view('media','admin/invoice_overlay_off',$data);
        }

        function printinvoiceoff($uid,$tax){
            $user= $this->usermodel->fetch('Officials', '*', null, array('id'=>$uid));
            $users = $user->row_array();
            $data['user'] = $user->row_array();
            $data['tax'] = ($tax == 'yes')?true:false;

            $this->load->module_view('media','admin/invoice_print_off',$data);
        }
