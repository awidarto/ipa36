<?php if (!defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Officials extends Admin_Controller
	{
		...
		function Officials()
		{
            ...
            $this->load->config('official');
            $this->load->dbutil();
            $this->load->helper('download');
            $this->controller = 'media/admin/officials';
            $this->section = 'Officials';
            $this->header = 'Official Participants';
            $this->where = null;
            $this->bep_site->set_crumb('Official Participants','media/admin/officials');
        ...
        function index()
        {
            ...
            $convql = ' ((sc_seq > 0 or conv_seq > 0) and users.group = 1) '; 
            ...
            */
            ...
            $query = $this->usermodel->getOfficials($this->where, null,array('fields'=>$orderby,'order'=>$sort)); 
            ...
            $query = $this->usermodel->getOfficials($this->where, $limit,array('fields'=>$orderby,'order'=>$sort));   
            ...
            if(($type == 'result')){
                $this->bep_site->set_crumb('Search Result','media/admin/officials');
            }
            ...
            $summary_list['Total Registered User'] = $taccount;
            ...
            if($type == 'xml'){
                ...
                $page = $this->config->item('backendpro_template_admin') . $selview;
                $this->load->view($page,$data);
            }else{
                $data['page'] = $this->config->item('backendpro_template_admin') . "officials_details";
                $data['module'] = 'media';
        ...
        function delusr()
    	{
			...
			{	// Delete as long as its not the Administrator account
				$this->usermodel->delete('Officials',array('id'=>$id));

				$user= $this->usermodel->fetch('Officials', '*', null, array('id'=>$id));
