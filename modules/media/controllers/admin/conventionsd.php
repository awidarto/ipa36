<?php if (!defined('BASEPATH')) exit('No direct script access allowed');
     ...
     */
	class Conventionsd extends Admin_Controller
	{
		/**
		 * Constructor
		 */
		function Conventionsd()
		{
            ...
            $this->controller = 'media/admin/conventionsd';
            $this->section = 'ConventionStudentDomestic';
            $this->header = 'Convention - Student Domestic';
            $this->where = null;
            $this->bep_site->set_crumb('Convention','media/admin/conventionsd');
        ...
        function index()
        {
            ...
            */
            ...
            $convql = ' ((conv_seq > 0) and foc = 0 and registrationtype = \'Student Domestic\' and users.group = 1) '; 
            ...
            if(($type == 'result')){
                $this->bep_site->set_crumb('Search Result','media/admin/conventionsd');
            }
            ...
            foreach($summary_obj as $so){
                ...
                $shortcourse = $so->course_1 + $so->course_2 + $so->course_3 + $so->course_4 + $so->course_5;
                
                if($so->conv_seq > 0 && $shortcourse == 0){
                    $co++;
                    $tco++;
                }
            ...
            $summary_list['Total Student Domestic Attendance'] = $tco;
