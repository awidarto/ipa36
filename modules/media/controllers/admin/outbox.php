<?php if (!defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Outbox extends Admin_Controller
	{
		...
		function Outbox()
		{
			...
			$this->load->model('Eventlogmodel','docs');
            ...
            $this->load->config('outbox');
            $this->load->dbutil();
            $this->load->helper('download');
            $this->controller = 'media/admin/outbox';
            $this->section = 'SystemOutbox';
            $this->header = 'System Outbox';
            $this->where = null;
            $this->bep_site->set_crumb('Convention','media/admin/outbox');
        ...
        function index()
        {
            ...
            $orderby = ($this->uri->segment(7))?$this->uri->segment(7):"timesent";
            ...
            $convql .= ' or ( registertype > 0 and (course_1 + course_2 + course_3 + course_4 + course_5 > 0))'; 
            $convql = ' (foc = 1 or (registrationtype = \'Professional Domestic\' or registrationtype = \'Professional Overseas\' or registrationtype = \'Student Domestic\' or registrationtype = \'Student Overseas\')) and users.group = 1 '; 
            ...
            */
            ...
            $qdf = $this->docs->getOldestDate('Outbox','timesent');
            $odate = $qdf->row();
            $odate = explode(' ',$odate->timesent);
            ...
            if(($type == 'result')){
                $this->bep_site->set_crumb('Search Result','media/admin/outbox');
            }
            ...
            if($type == 'xml'){
                ...
                $page = $this->config->item('backendpro_template_admin') . 'outbox_list';
                $this->load->view($page,$data);
            }else{
                $data['page'] = $this->config->item('backendpro_template_admin') . 'outbox_list';
                $data['module'] = 'media';
