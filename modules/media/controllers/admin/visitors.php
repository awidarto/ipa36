<?php if (!defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Visitors extends Admin_Controller
	{
		...
		function Visitors()
		{
            ...
            $this->load->config('visitor');
            $this->load->dbutil();
            $this->load->helper('download');
            $this->controller = 'media/admin/visitors';
            $this->section = 'Visitors';
            $this->header = 'Visitors';
            $this->where = null;
            $this->bep_site->set_crumb('Visitors','media/admin/visitors');
        ...
        function index()
        {
            ...
            if($searchscope != "none" && $searchstring != ""){
                if($searchscope == "all"){
                    ...
                    //$this->where.= " ".$searchscope." like '%".$searchstring."%'";
                }
            }
            ...
            */
            ...
            $query = $this->usermodel->getVisitors($this->where, null,array('fields'=>$orderby,'order'=>$sort)); 
            ...
            $query = $this->usermodel->getVisitors($this->where, $limit,array('fields'=>$orderby,'order'=>$sort));   
            ...
            if(($type == 'result')){
                $this->bep_site->set_crumb('Search Result','media/admin/visitors');
            }
            ...
            if($type == 'xml'){
                ...
                $page = $this->config->item('backendpro_template_admin') ."visitors_details";
                $this->load->view($page,$data);
            }else{
                $data['page'] = $this->config->item('backendpro_template_admin') . "visitors_details";
                $data['module'] = 'media';
        ...
        function delusr()
    	{
			...
			{	// Delete as long as its not the Administrator account
				$this->usermodel->delete('Visitors',array('id'=>$id));

				$user= $this->usermodel->fetch('Visitors', '*', null, array('id'=>$id));
