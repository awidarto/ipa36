<?php if (!defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Attendance extends Admin_Controller
	{
		...
		function Attendance()
		{
            ...
            $this->controller = 'media/admin/attendance';
            $this->section = 'Convention Attendance';
            $this->header = 'Convention Attendance Log';
            $this->where = null;
            $this->bep_site->set_crumb('Convention Attendance Log','media/admin/attendance');
        ...
        function index()
        {
            ...
            $convql .= ' or ( registertype > 0 and (course_1 + course_2 + course_3 + course_4 + course_5 > 0))'; 
            */
            $convql = " (users.registrationtype = 'Attendee' ) "; 
            ...
            $query = $this->usermodel->getUserAttendances($this->where, null,array('fields'=>$orderby,'order'=>$sort));   
            ...
            $query = $this->usermodel->getUserAttendances($this->where, $limit,array('fields'=>$orderby,'order'=>$sort));   
            ...
            if(($type == 'result')){
                $this->bep_site->set_crumb('Search Result','media/admin/attendance');
            }
            ...
            if($type == 'xml'){
                ...
                $data['page'] = $this->config->item('backendpro_template_admin') . "attendance_list_detailed";
                $data['module'] = 'media';
