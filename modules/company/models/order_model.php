 ...
 */
    class Order_model extends Base_model
    {
        ...
        function Order_model()
        {
            ...
            $this->_TABLES = array('Documents' => 'ci_media_order','Routes'=>'ci_doc_routes','Custodian'=>'ci_doc_custodian','LocationMain'=>'ci_loc_main','Companies'=>'ci_companies','CallNumbers'=>'ci_call_main','Users'=>'be_users','Tags'=>'ci_tag','SectionTags'=>'ci_object_tag','Media'=>'ci_media_library','Files'=>'ci_files','UserProfiles'=>'be_user_profiles');
        ...
        function getClients($username = null)
    	{	
    		$this->db->select('*');
    		$this->db->where(array('group'=>3));
    		if(!is_null($username)){
        		$this->db->where(array('username'=>$username));
    		}
    		return $this->db->get($this->_TABLES['Users']);
        ...
        function getClientMedia($fid = null,$type = null)
    	{	
    		$this->db->select('*');
    		if(!is_null($fid)){
        		$this->db->where(array('ownerid'=>$fid));
    		}
    		if(!is_null($type)){
        		$this->db->where(array('mediatype'=>$type));
    		}
    		return $this->db->get($this->_TABLES['Media']);
    		//returns the query string
    	}
