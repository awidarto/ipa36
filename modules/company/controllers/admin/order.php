<?php if (!defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Order extends Admin_Controller
	{
		...
		function Order()
		{
			...
			$this->load->model('Order_model','docs');
            ...
            $this->controller = 'media/admin/order';
            $this->section = 'DC';
            $this->where = null;
            $this->page->set_crumb('Orders','media/admin/order');
        ...
        function index()
        {
            ...
            */
            ...
            $fields = 'id,ordername,clientid,mediaid,mediatype,monthstart,monthend,yearstart,yearend,descriptions,datecreated';            
            ...
            if($type == 'xml'){
                ...
                $page = $this->config->item('backendpro_template_admin') . "order_list";
                $this->load->view($page,$data);
            }else{
                $data['page'] = $this->config->item('backendpro_template_admin') . "order_list";
                $data['module'] = 'media';
        ...
        function form($id = NULL)
        {
            ...
            foreach($this->config->item('order_validation_array') as $key=>$val){
            ...
            if( !is_null($id) && ! $this->input->post('submit'))
            {
                ...
                $clients = $this->_getClients()->result();
                
                foreach($clients as $c){
                    $cl[$c->id] = $c->firstname.' '.$c->lastname.' ['.$c->username.']';
                }
                
                $data['clients'] = $cl;
                
                $media = $this->_getClientMedia();
                
                foreach($media as $m){
                    $mi[$m->id] = basename($m->filename).' ['.$m->duration.']';
                }
            ...
            if ($this->validation->run() === FALSE)
            {
                ...
                $data['header'] = (is_null($id))?'New Order':'Edit Order : '.$id;
                $this->page->set_crumb($data['header'],'documents/form/'.$id);
                $data['page'] = $this->config->item('backendpro_template_admin') . "form_order";
                $data['module'] = 'media';
                $data['controller'] = $this->controller;
                $this->load->view($this->_container,$data);                 
            }
        ...
        function getmedia($ownerid){
            if($media = $this->_getClientMedia($ownerid)){
                $out = "";
                foreach($media as $m){
                    $out .= '<option value="'.$m->id.'">'.basename($m->filename).' ['.$m->duration.']</option>';
                }
                print($out);
            }else{
                print('<option value="na" >no media found</option>');
            }
        }
        ...
        function _getClientMedia($cid = null,$type = null){
            $query = $this->docs->getClientMedia($cid,$type = null);
            if($query->num_rows() > 0){
                return $query->result();
            }else{
                return false;
