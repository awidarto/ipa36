<?php if (!defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Media extends Admin_Controller
	{
        ...
        function contactlog($uid){
			...
			//$data['qrfile'] = $this->_generateQR($usr['id'].'_qr.png',$data['user']);
        ...
        function userlist($company){
            ...
            $query = $this->usermodel->getUsers("company ='".base64_decode($company)."'");   
            $usr = $query->result_array();
            foreach($usr as $u){
                $u['picture'] = get_avatar($u['id'],$this->config->item('public_folder').'avatar/');            
                $user[] = $u;
            }
            
            $data['user'] = $user;
            $data['company'] = base64_decode($company);
            $this->load->module_view('company','admin/user_overlay',$data);
        }        
